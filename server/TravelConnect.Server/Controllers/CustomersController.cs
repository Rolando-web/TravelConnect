...
public class CustomersController(TravelConnectDbContext db) : ControllerBase
{
    [HttpGet]
    ...
    {
        var entity = await db.Customers.AsNoTracking().FirstOrDefaultAsync(e => e.Id == id);
    ...
    public async Task<IActionResult> Update(int id, Customer entity)
    {
        ...
        var existing = await db.Customers.FirstOrDefaultAsync(e => e.Id == id);
    ...
    {
        var entity = await db.Customers.FirstOrDefaultAsync(e => e.Id == id);
        if (entity is null) return NotFound(new { message = "Record not found" });
