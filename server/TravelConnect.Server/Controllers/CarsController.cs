...
public class CarsController(TravelConnectDbContext db) : ControllerBase
{
    [HttpGet]
    [AllowAnonymous]
    ...
    {
        var entity = await db.Cars.AsNoTracking().FirstOrDefaultAsync(e => e.Id == id);
        if (entity is null) return NotFound(new { message = "Record not found" });
        return Ok(entity);
    }

    [HttpPost]
    public async Task<ActionResult<Car>> Create(Car entity)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update(int id, Car entity)
    {
        if (id != entity.Id) return BadRequest(new { message = "ID mismatch" });
        var existing = await db.Cars.FirstOrDefaultAsync(e => e.Id == id);
    ...
    {
        var entity = await db.Cars.FirstOrDefaultAsync(e => e.Id == id);
        if (entity is null) return NotFound(new { message = "Record not found" });
