...
export default function DateRangeCalendar({
  ...
  const MonthGrid = ({ mInfo }) => (
    <div>
      <p className="text-center font-extrabold text-sm text-gray-900 mb-3">{mInfo.label}</p>
      ...
      <div className="grid grid-cols-7 gap-1 mt-2 text-center text-xs">
        {getCalendarMonthDays(mInfo.year, mInfo.monthIndex).map((day, idx) => (
          <button
            ...
            className={`h-8 w-8 rounded-full font-bold transition flex items-center justify-center ${
              !day
                ? "bg-transparent text-transparent pointer-events-none"
                : isSelected(day, mInfo)
                ? "bg-[#008fe5] text-white shadow-sm"
                : isInRange(day, mInfo)
                ? "bg-blue-50 text-[#008fe5] rounded-none"
  ...
  return (
    <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 shadow-2xl rounded-3xl p-6 z-[90] w-[600px] max-w-[92vw] text-left">
      ...
      <div className="flex items-center justify-between px-2 mb-2">
        <button
          type="button"
          onClick={prevMonth}
          className="p-1.5 rounded-full hover:bg-gray-100 text-gray-700 transition flex items-center gap-1 text-xs font-bold"
      ...
      {/* Footer */}
      <div className="flex flex-col sm:flex-row items-center justify-between border-t border-gray-100 pt-4 mt-4 gap-3">
        <span className="text-xs text-gray-500 font-medium">
          {departureDate ? (
            <>
              Depart: <span className="font-bold text-gray-900">{formatDateLabel(departureDate)}</span>
              {returnDate && <> · Return: <span className="font-bold text-gray-900">{formatDateLabel(returnDate)}</span></>}
            </>
          ) : (
        ...
        <button
          type="button"
          onClick={onClose}
          className="w-full sm:w-auto bg-[#008fe5] hover:bg-blue-600 text-white font-bold px-6 py-2 rounded-xl text-xs shadow-md transition"
        >
