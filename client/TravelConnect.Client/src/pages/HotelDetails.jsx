...
import {
  Hotel, Star, MapPin, ArrowLeft, CheckCircle2, ShieldCheck, Sparkles, Wifi, Coffee, Waves, Car
} from "lucide-react";
import { getHotelById, toHotelBooking } from "../data/hotelsData";
...
export default function HotelDetails() {
  ...
  const gallery = hotel.gallery?.length ? hotel.gallery : [hotel.img];
  const total = hotel.pricePerNight * NIGHTS;
  const originalTotal = hotel.originalPrice * NIGHTS;
    ...
    <div className="w-full bg-slate-50 min-h-screen pb-16">
      ...
      <div className="relative text-white overflow-hidden">
        <div className="absolute inset-0">
          <img
            src={gallery[activeImg] || hotel.img}
            alt={hotel.name}
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-14">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
            <div className="lg:col-span-2 space-y-4">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <span className="bg-white/10 border border-white/20 text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1.5">
                  <Hotel size={14} className="text-amber-400" /> {hotel.stars}★ Resort
                </span>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-4 text-xs sm:text-sm text-slate-200 font-medium">
                <span className="flex items-center gap-1 text-amber-400 font-bold">
                  <Star size={16} className="fill-amber-400" /> {hotel.rating} ({hotel.reviews} reviews)
                </span>
                <span>•</span>
            ...
            <div className="bg-white text-slate-900 rounded-3xl p-6 shadow-2xl border border-slate-100 space-y-4">
              ...
              <div>
                <span className="text-xs text-slate-400 line-through">₱{hotel.originalPrice.toLocaleString()}/night</span>
                <div className="flex items-baseline gap-1">
                  <span className="text-3xl font-black text-slate-900">₱{hotel.pricePerNight.toLocaleString()}</span>
              ...
              <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100">
                <p className="text-[10px] text-slate-400 font-bold uppercase">Room Type</p>
                <p className="text-sm font-extrabold text-slate-900">{hotel.roomType}</p>
              </div>

              <button
                onClick={() => openCheckoutModal(toHotelBooking(hotel, NIGHTS))}
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-3">
              <div className="relative rounded-3xl overflow-hidden shadow-lg h-72 sm:h-96">
                <img src={gallery[activeImg]} alt={hotel.name} className="w-full h-full object-cover" />
              </div>
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-sm border border-slate-200/80 space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {hotel.amenities.map((am) => (
                  <div key={am} className="flex items-center gap-2 text-sm text-slate-700 font-medium p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <CheckCircle2 size={16} className="text-emerald-500 shrink-0" /> {am}
                  </div>
                ))}
              </div>
              <div className="flex flex-wrap gap-4 pt-2 text-xs text-slate-500 font-semibold">
                <span className="flex items-center gap-1.5"><Wifi size={14} className="text-[#008fe5]" /> High-speed Wi‑Fi</span>
                <span className="flex items-center gap-1.5"><Coffee size={14} className="text-[#008fe5]" /> Breakfast options</span>
                <span className="flex items-center gap-1.5"><Waves size={14} className="text-[#008fe5]" /> Beach / pool access</span>
                <span className="flex items-center gap-1.5"><Car size={14} className="text-[#008fe5]" /> Transfer available</span>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white rounded-3xl p-6 shadow-xl border border-slate-200/80 sticky top-24 space-y-5">
              ...
              <div className="space-y-3 text-xs">
                <div className="flex items-center justify-between text-slate-600 font-medium">
                  <span>₱{hotel.originalPrice.toLocaleString()} × {NIGHTS} nights</span>
